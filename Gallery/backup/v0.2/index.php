	...
	<body>
 		<div id="gallery-wrapper" class="gallery">
			...
			<div class="thumbnails">
				...
				$pageCount = count( $imageObjects );

				//error handling when the user reaches the max pages either manually or dynamically . validation points  --> 
				if( $page < 1 )
					$page = 1;
				elseif( $page > intval( $pageCount / $itemsPerPage ) )
					$page = intval( $pageCount / $itemsPerPage );
			...
			<div>
				<div class="pagination">
					...
					?>
					<a href="http://localhost:8888/Gallery/?page=<?= $previousPage ?>">&laquo;</a>

					<?php
					$offset = intval( $pagesPerPage / 2 );
					if( $page <= $offset ) $offset = $page - 1;
					if( $page >= $pageCount - $offset ) $offset = $pageCount - $offset;
					...
					?>
					<a href="http://localhost:8888/Gallery/?page=<?= $nextPage ?>">&raquo;</a>
				</div>
			</div>
		</div> 
